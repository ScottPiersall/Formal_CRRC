...
class Runner:
 def __init__(self,stage):
  ...
  gate=load(OUT/'costs/submission_gate.json');self.job=os.environ['SLURM_JOB_ID'];self.deadline=float(os.environ['FOLLOWUP_DEADLINE_EPOCH'])
  assert gate['job_id']==self.job and gate['stage']==stage and gate['quota_gate_passed'] and gate['project_budget_gate_passed']
  assert gate['reserved_physical_gpu_seconds']<=360000 and 0<self.deadline-time.time()<=gate['reserved_physical_gpu_seconds']
  ...
  if stage=='main':assert load(OUT/'costs/smoke_gate.json')['technical_gate_passed']
 ...
 def save(self,output):
  ...
  if needed is not None:
   result.update(prefix=prefix,next_log_probabilities={str(i):float(c.logprobs[0][i].logprob) for i in needed})
  elif r['experiment']=='A':
 ...
 def score(self,r,t):
  ...
  if dest.exists():return
  try:
   ctx=t['final_context_token_ids'];paths,forms=answer_paths(self.tok,t['final_context_text'],ctx);trie=shared.trie(paths);nodes={};seconds=0.
   for prefix,needed in trie.items():
    ...
    sr=dict(r,prompt_token_ids=ctx);n=self.request(sr,p,list(prefix),needed)
    nodes[prefix]={int(k):v for k,v in n['next_log_probabilities'].items()};seconds+=n['elapsed_seconds']
   scores,events=shared.combine(forms,nodes)
   result=dict(status='ok',scores=scores,answer_events=events,prefix_nodes=[dict(token_ids=list(k),next_log_probabilities=v) for k,v in nodes.items()],scoring_seconds=seconds,extra_scoring_generated_tokens=len(nodes))
 ...
 def run(self):
  ...
  for r in todo:
   ...
   if self.stage=='main':
    seen=[load(OUT/'traces'/(x['condition_id']+'.json')) for x in todo if (OUT/'traces'/(x['condition_id']+'.json')).exists()]
    if len(seen)>=30 and sum(x['status']!='ok' for x in seen)>len(seen)/2:
     write(OUT/'costs/output_failure_stop.json',dict(job_id=self.job,at=now(),processed=len(seen),failed=sum(x['status']!='ok' for x in seen)),True);break
   t=self.request(r,OUT/'traces'/(r['condition_id']+'.json'))
   if r['experiment']=='A' and t['status']=='ok':self.score(r,t)
   print(r['condition_id'],t['status'],t['generated_tokens'],round(self.remaining()),flush=True)
  # A separate CPU gate aggregates all persisted smoke requests and timing.
